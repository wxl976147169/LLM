...
def correctness_reward(prompts, completions, answer, **kwargs):
    ...
    extracted_responses = [extract_answer(r) for r in responses]
    rewards = []
    for resp, gt in zip(extracted_responses, answer):
        try:
            reward = 2.0 if resp.strip() == str(gt).strip() else 0.0
        except Exception:
            reward = 0.0
...
if __name__ == '__main__':
    model_name = "/root/autodl-tmp/LLM/Qwen2.5-0.5B-Instruct"
    ...
    ds = load_dataset('/root/autodl-tmp/LLM/gsm8k_chinese')
    data = process_data(ds['train'])
    
    output_dir="/root/autodl-tmp/LLM/output"
